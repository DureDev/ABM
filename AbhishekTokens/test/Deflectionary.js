...
describe("Deflectionary", function () { 
  let owner;
  let addr1, addr2;
  ...
  beforeEach(async function () {
    ...
    Abhishek = await ethers.getContractFactory("Deflectionary");
  ...
  it("Should transfer tokens to other account ", async function () {
   ...
   it("Should emit an event", async function () {
    ...
    .to.emit(hardhatAbhishek, 'Transfer').withArgs(owner.address, addr1.address, 99, 1);
  ...
  it("Allowed user should spend his allowance not more than that", async function () {
    ...
    expect(await hardhatAbhishek.balanceOf(addr2.address)).to.equal(198);
